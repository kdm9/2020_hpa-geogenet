...
```{r ibd}
ibd.geo.dist = as.matrix(eu.geo[, c("longitude", "latitude")]) %>%
    earth.dist() %>%
    as.dist()
ibd.geno.dist = geno.dist %>%
    as.dist()
vegan::mantel(ibd.geno.dist, ibd.geo.dist)
mg = ecodist::mgram(ibd.geno.dist, ibd.geo.dist)
mg$mgram[,4]
...
save.image(file="hpa.Rda", compress="xz")
```
