...
ls()
load("out/Hpa_cs.xval.data.partitions.Robj")
load("out/Hpa_cs.xval.results.Robj")
xv = x.val %>%
    unlist(recusive=F) %>%
    unlist(recusive=F) %>%
    do.call(
...
n.reps=4
K=1:6
res.tib = foreach::foreach(rep.no=1:n.reps, .combine=bind_rows) %:%
          foreach::foreach(k=K, .combine=bind_rows) %:%
          foreach::foreach(mdl=c("sp", "nsp"), .combine=bind_rows) %do% {
    spi = 1
    if (mdl=="nsp") spi = 2
    cat(paste("rep", rep.no, "K", k, mdl, "\n"))
    as.tibble(x.val[[rep.no]][[k]][[spi]])
}
res.tib
```
...
cxv.tmp
%>%
    mutate(mdl.fit=conStruct:::fit.to.test(data.part$testing, construct.res))
```
